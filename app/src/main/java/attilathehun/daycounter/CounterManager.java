...
public class CounterManager {
    ...
    public static final String DEFAULT_EXPORT_PATH = "/storage/emulated/0/counters.json";
    public static final String SAVE_FILE_PATH = Util.getContext().getFilesDir() + "/data.json";
    @Deprecated
     ...
     */
    private static void notifyCounterRemoved(final Counter counter) {
    ...
    public boolean addCounter(final String name, final int targetDay, final int targetMonth, final int targetYear, final int targetAge) {
        ...
        final Counter temp = Counter.create(id, name, targetDay, targetMonth, targetYear, targetAge);
    ...
    public void save() {
        try {
            ...
            final File dataFile = new File(SAVE_FILE_PATH);
            dataFile.createNewFile();
            final FileOutputStream out = new FileOutputStream(dataFile, false);
    ...
    public void load() {  
        try {
            ...
            final byte[] bytes = new byte[(int) dataFile.length()];
            in.read(bytes);
     ...
     */
    public Counter getWidgetCounterForId(final int widgetId) {
        for (Counter counter : counters) {
     ...
     * @param widgetId  target widget id
     */
    public void bindWidget(final String counterId, final int widgetId) {
        for (int i = 0; i < counters.size(); i++) {
     ...
     * @param counterId target counter id
     */
    public void unbindWidget(final String counterId) {
        for (int i = 0; i < counters.size(); i++) {
            if (counters.get(i).getId().equals(counterId)) {
     ...
     * @param widgetId target widget id
     */
    public void unbindWidgetOfId(final int widgetId) {
        for (int i = 0; i < counters.size(); i++) {
     ...
     */
    public Counter getCounterOfId(final String counterId) {
        for (int i = 0; i < counters.size(); i++) {
            if (counters.get(i).getId().equals(counterId)) {
     ...
     * @return true if successful
     */
    public boolean exportJSON(final String path) {
        try {
            if (counters.size() == 0) {
                SketchwareUtil.showMessage(Util.getContext(), Util.getContext().getString(R.string.no_counter_available));
                return false;
            ...
            exportFile.createNewFile();
            final FileOutputStream out = new FileOutputStream(exportFile, false);
            out.write(jsonData.getBytes("UTF-8"));
            out.close();
            Util.log("Exported to " + path);
            return true;
    ...
    public boolean exportJSONDefault() {
        return exportJSON(DEFAULT_EXPORT_PATH);
    }
     ...
     * @return true if successful
     */
    public boolean importJSON(final String path) {
        try {
            final File importFile = new File(path);
            final FileInputStream in = new FileInputStream(importFile);
            final byte[] bytes = new byte[(int) importFile.length()];
            in.read(bytes);
     ...
     */
    public void duplicate(final Counter counter) {
     ...
     * @return true if successful
     */
    public boolean editCounter(final String id, final String name, final int targetDay, final int targetMonth, final int targetYear, final int targetAge) {
        for (int i = 0; i < counters.size(); i++) {
            if (counters.get(i).getId().equals(id)) {
    ...
    private void backup() {
        if (AUTOMATIC_BACKUP) {
            this.exportJSON(CounterManager.SAVE_FILE_PATH);
        }
    }
    ...
    private void restore() {
        if (AUTOMATIC_BACKUP) {
            this.importJSON(CounterManager.SAVE_FILE_PATH);
        }
    }
