...
public class CreateCounterActivity extends AppCompatActivity {
            ...
            public void onClick(View _view) {
                if (enter_year.getText().toString().equals("")) {
                    ...
                    if (enter_target_age.getText().toString().equals("")) {
                        ...
                        boolean success;
                        if (edit) {
                            success = CounterManager.getInstance().editCounter(counterId, name, targetDay, targetMonth, targetYear, targetAge);
                            intent.putExtra("position", getIntent().getIntExtra("position", 0));
                        } else {
                            success = CounterManager.getInstance().addCounter(name, targetDay, targetMonth, targetYear, targetAge);
                        }
                        if (success) {
    ...
    public void _initTranslation() {
        ...
        if (edit) {
            create_button.setText(getResources().getString(R.string.edit_counter_label));
        } else {
    ...
    public void _initEditCounter() {
        if (!getIntent().getStringExtra("name").equals("\"N/A\"")) {
            name_box.setText(getIntent().getStringExtra("name"));
        }
        try {
            counterId = getIntent().getStringExtra("id");
            day_selector.setSelection(getIntent().getIntExtra("targetDay", 0));
            month_selector.setSelection(getIntent().getIntExtra("targetMonth", 0));
            enter_year.setText(String.valueOf(getIntent().getIntExtra("targetYear", 0)));
            enter_target_age.setText(String.valueOf(getIntent().getIntExtra("targetAge", 0)));
        } catch (Exception e) {
