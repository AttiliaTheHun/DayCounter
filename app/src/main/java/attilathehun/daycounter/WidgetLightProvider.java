...
public class WidgetLightProvider extends AppWidgetProvider {
    ...
    public static void refresh(Context context) {
        Intent intent = new Intent(context, WidgetLightProvider.class);
        intent.setAction(AppWidgetManager.ACTION_APPWIDGET_UPDATE);
        // Use an array and EXTRA_APPWIDGET_IDS instead of AppWidgetManager.EXTRA_APPWIDGET_ID,
        // since it seems the onUpdate() is only fired on that:
        int[] ids = AppWidgetManager.getInstance(context).getAppWidgetIds(new ComponentName(context, WidgetLightProvider.class));
